<?php
/**
 * The Tutorials Custom Post Type & Taxonomies.
 *
...
function tutorials_register() {
	...
	$labels = array(
		'name' => 			__('Tutorials', 'ash'),
		'singular_name' => __('Tutorials', 'ash'),
		'add_new' => __('Add New Tutorial', 'ash'),
		'add_new_item' => __('Add New Tutorial','ash'),
		'edit_item' => __('Edit Tutorial','ash'),
		'new_item' => __('New Tutorial','ash'),
		'view_item' => __('View Tutorial','ash'),
		'search_items' => __('Search Tutorial','ash'),
	...
	$args = array(
		...
		'menu_icon' => get_template_directory_uri() . '/library/xtra/post-types/tutorials/tutorials18.png',
		'rewrite' => true,
...
/*-----------------------------------------------------------------------------------*/
	...
	register_taxonomy( 'tut-cat', 'tutorials',
				  	array(	"hierarchical" => true,
							"label" => __('Type of Tutorial','ash'),
							"rewrite" => true,
							"query_var"=>true
						  )
					);
...
function tutorials_updated_messages( $messages ) {
  global $post, $post_ID;

  $messages['tutorials'] = array(
...
add_filter( 'post_updated_messages', 'tutorials_updated_messages' );
...
function tutorials_add_help_text( $contextual_help, $screen_id, $screen ) { 
  //$contextual_help .= var_dump( $screen ); // use this to help determine $screen->id
  if ( 'tutorials' == $screen->id ) {
    $contextual_help =
...
add_action( 'contextual_help', 'tutorials_add_help_text', 10, 3 );
...
function change_tutorials_cpt_default_title( $title ){
     $screen = get_current_screen();
 
     if  ( 'tutorials' == $screen->post_type ) {
          $title = 'The Name of this Tutorial';
     }
 
     return $title;
}
add_filter( 'enter_title_here', 'change_tutorials_cpt_default_title' );
...
add_action( 'admin_head', 'ash_tutorials_icons' );
function ash_tutorials_icons() {
    ?>
    <style type="text/css" media="screen">
		#icon-edit.icon32-posts-tutorials {background: url(<?php get_template_directory_uri(); ?>/library/xtra/post-types/tutorials/tutorials32.png) no-repeat;}
    </style>
<?php }
